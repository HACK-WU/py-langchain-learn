...
> 无头 React AI 聊天框架，带有完整的运行时层，通过 useStream 桥接

assistant-ui 是一个无头 React UI 框架，用于构建 AI 聊天界面。它提供了一个完整的运行时层——包括线程管理、消息分支、附件处理——通过 `useExternalStoreRuntime` 适配器连接到 `useStream`。

克隆并运行完整的 assistant-ui 示例，查看一个连接到 LangChain 代理的 Claude 风格聊天界面，使用 `useExternalStoreRuntime` 进行桥接。

## 工作原理

1. 使用 `useStream` 进行流式传输 —— 连接到你的代理，获取响应式消息、加载状态和提交/取消回调
2. 使用 `useExternalStoreRuntime` 进行适配 —— 通过将 `BaseMessage[]` 转换为 `ThreadMessageLike[]`，将 `stream.messages` 桥接到 assistant-ui 的运行时格式
3. 提供运行时 —— 使用 `AssistantRuntimeProvider` 包装你的 UI，并渲染任何 assistant-ui 线程组件
...
`Thread` 组件提供了一个完整的默认线程 UI，包括消息列表、输入框和滚动管理。通过覆盖组件插槽来自定义各个部分：
...
- **缓存消息转换**：将 `toThreadMessages(stream.messages)` 包装在 `useMemo` 中，以避免在每次渲染时重新运行转换
- **处理附件**：使用 `CompositeAttachmentAdapter` 配合 `SimpleImageAttachmentAdapter` 进行图片上传；使用自定义适配器扩展以支持文件
- **使用分支**：assistant-ui 通过 `MessageBranch` 内置支持消息分支；编辑消息以从该点重新生成
- **线程持久化**：配合 `fetchStateHistory: true` 和 `reconnectOnMount: true` 使用 `useStream`，使 assistant-ui 在页面加载时可以访问完整的线程历史记录
